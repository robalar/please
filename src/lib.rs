...
pub trait ConnectionProvider {
    ...
    type Connection: LoadConnection<Backend = Pg>;
...
impl<P: ConnectionProvider> PleaseHandle<P> {
    ...
    pub fn new(provider: P, title: &str) -> PleaseResult<Self, P::Error> {
        ...
        // Allocate a new ID
        let id: i32 =
            Self::transaction_internal(&provider, |conn| -> PleaseResult<i32, P::Error> {
    ...
    pub fn new_with_connection<C>(provider: P, title: &str, conn: &mut C) -> QueryResult<Self>
    where
        ...
        F: FnOnce(&P::Connection, i32) -> Result<R, E>,
    {
        ...
        Self::transaction_internal(&self.provider, |conn| {
            ...
            // it hasn't expired.
            let num_rows = diesel::update(please_ids::table.filter(please_ids::id.eq(self.id)))
    ...
    pub fn expire(&mut self) -> PleaseResult<ExpiredId, P::Error> {
        ...
        Self::transaction_internal(&self.provider, |conn| {
            diesel::delete(please_ids::table.filter(please_ids::id.eq(self.id)))
                .get_result(conn)
                .optional()?
...
mod tests {
    ...
    fn new_handle(name: &str) -> PleaseHandle<TestConnectionProvider> {
        PleaseHandle::new(TestConnectionProvider, name).expect("Failed to create handle")
    }

    fn new_handle_with_connection(name: &str) -> PleaseHandle<TestConnectionProvider> {
        let mut conn = TestConnectionProvider
            .get()
    ...
    fn smoke() {
        ...
        handle
            .transaction(|_conn, _id| Ok::<(), PleaseError<_>>(()))
    ...
    fn two_handles() {
        ...
        handle1.close().expect("Failed to close handle 1");
        handle2.close().expect("Failed to close handle 2");
    }
